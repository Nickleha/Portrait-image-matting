'''
Helper functions repurposed from udacity
Source code: https://github.com/udacity/CarND-Semantic-Segmentation/blob/master/helper.py
Minor edits made to remove unnecessary functions and match naming conventions used in main script. 
'''
...
def gen_test_output(sess, logits, keep_prob, image_pl, data_folder, image_shape):
	...
	for image_file in glob(os.path.join(data_folder, '*.jpg')):
		...
		# If road softmax > 0.5, prediction is person
...
def save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, keep_prob, input_image):
	...
	image_outputs = gen_test_output(
		sess, logits, keep_prob, input_image, os.path.join(data_dir, 'splits/test_small'), image_shape)
